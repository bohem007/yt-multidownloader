...
def test_log_job_start_and_finish_roundtrip(db):
    ...
    client_hash = f"testhash-{uuid.uuid4().hex}"

    job_id = db.log_job_start(
        marker_url, mode="audio", output_format="mp3", client_ip_hash=client_hash
    )
    assert isinstance(job_id, int)

    db.log_job_finish(job_id, status="done", duration_ms=1234, file_size_bytes=5678)

    history = db.get_recent_history(client_hash, 5, limit=50)
    matching = [row for row in history if row["id"] == job_id]
    assert len(matching) == 1
    # Historia jest filtrowana po hashu klienta, a sama kolumna nie jest zwracana.
    assert db.get_recent_history(f"other-{uuid.uuid4().hex}", 5, limit=50) == []
    ...
    assert "client_ip_hash" not in row
...
def test_log_job_finish_records_error_message(db):
    ...
    client_hash = f"testhash-{uuid.uuid4().hex}"
    job_id = db.log_job_start(
        marker_url, mode="video", output_format="mp4", client_ip_hash=client_hash
    )
...
def test_get_recent_history_orders_by_created_at_desc(db):
    ...
    client_hash = f"testhash-{uuid.uuid4().hex}"
    first_id = db.log_job_start(
        first_url, mode="video", output_format="mp4", client_ip_hash=client_hash
    )
    second_id = db.log_job_start(
        second_url, mode="video", output_format="mp4", client_ip_hash=client_hash
    )

    history = db.get_recent_history(client_hash, 5, limit=2)
